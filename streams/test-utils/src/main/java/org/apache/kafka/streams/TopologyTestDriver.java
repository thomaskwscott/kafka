    ...
    private void captureOutputRecords() {
        ...
        for (final ProducerRecord<byte[], byte[]> record : output) {
            ...
            if (internalTopics.contains(outputTopicName)
                ...
                enqueueTaskRecord(record.topic(), topicPartition, record.timestamp(), record.key(), record.value(), record.headers());
            } else if (globalPartitionsByTopic.containsKey(outputTopicName)) {
                processGlobalRecord(record.topic(), record.timestamp(), record.key(), record.value(), record.headers());
            }
        }
    }
